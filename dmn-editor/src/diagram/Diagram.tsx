  ...
  ({ container }, ref) => {
      ...
      async (e: React.DragEvent) => {
        ...
        if (e.dataTransfer.getData(MIME_TYPE_FOR_DMN_EDITOR_NEW_NODE_FROM_PALETTE)) {
          ...
          if (nodeType === undefined) {
            throw new Error("DMN 图表：无法确定节点类型");
          }
      ...
      (e: React.KeyboardEvent) => {
        ...
        if (e.key === "Escape") {
          if (s.computed(s).isDiagramEditingInProgress() && dmnModelBeforeEditingRef.current) {
            console.debug(
              "DMN 图表：截获 Escape 按键并阻止传播。将 DMN 模型还原为编辑开始前的状态。"
            );
            ...
            dmnEditorStoreApi.setState((state) => {
              if (
                ...
                console.debug("DMN 图表：按下 Esc 键。取消选择所有内容。");
                ...
                console.debug("DMN 图表：按下 Esc 键。关闭所有打开的面板。");
    ...
    return (
      <>
        ...
        {isEmptyStateShowing && !canAutoGenerateDrd && <DmnDiagramEmptyState setShowEmptyState={setShowEmptyState} />}
      ...
      <div className={"kie-dmn-editor--diagram-empty-state"}>
        ...
        <EmptyState variant={EmptyStateVariant.small}>
          ...
          <EmptyStateBody style={{ fontSize: "12px", wordBreak: "break-word" }}>
            自动生成图表将使用默认大小和形状自动放置节点。您也可以
            使用工具栏中的&quot;DRG节点&quot;选项手动构建图表。
          </EmptyStateBody>
        </EmptyState>
      </div>
    </Bullseye>
    ...
    >
      <div className={"kie-dmn-editor--diagram-empty-state"}>
        ...
        <EmptyState>
          ...
          <EmptyStateBody>通过从工具栏拖拽节点开始</EmptyStateBody>
          <br />

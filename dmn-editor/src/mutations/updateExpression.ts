...
export function updateExpression({
  ...
  if (!drgElement) {
    throw new Error("DMN 变更：无法为不存在的 drgElement 更新表达式。");
  }
  ...
  if (drgElement?.__$$element === "decision") {
    ...
    if (expression.__$$element !== "functionDefinition") {
      throw new Error("DMN 变更：不能在不是函数的 BKM 上设置表达式。");
    }

    if (!expression?.__$$element) {
      throw new Error("DMN 变更：无法在没有 __$$element 属性的情况下确定表达式类型。");
    }
    ...
    throw new Error("DMN 变更：无法为不是决策或 BKM 的 drgElement 更新表达式。");
  }
}

...
export function deleteNode({
  ...
  if (
    ...
    console.warn("DMN 变更：无法从 DRD 中隐藏被决策服务包含的决策。");
  ...
  if (!__readonly_dmnObjectQName.prefix) {
    ...
    if (__readonly_nodeNature === NodeNature.ARTIFACT) {
      if (mode === NodeDeletionMode.FROM_DRG_AND_ALL_DRDS) {
        ...
        throw new Error(`DMN 变更：无法隐藏工件节点。`);
      }
      ...
      throw new Error(`DMN 变更：未知的节点类型 '${__readonly_nodeNature}'。`);
    }
    ...
    if (!deletedDmnObject && __readonly_nodeNature !== NodeNature.UNKNOWN) {
       ...
       */
      throw new Error(`DMN 变更：无法删除不存在的 DMN 对象：ID=${__readonly_dmnObjectId}`);
    }
  }

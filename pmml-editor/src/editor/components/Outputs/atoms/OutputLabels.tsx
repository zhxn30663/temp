...
export const OutputLabels = (props: OutputLabelsProps) => {
  ...
  return (
    <>
      {optype && OutputFieldLabel("操作类型", optype)}
      {targetFieldValidation.length > 0 ? (
        ...
        targetField && OutputFieldLabel("目标字段", targetField)
      )}
      {feature && OutputFieldLabel("特征", feature)}
      {value && OutputFieldLabel("值", value)}
      {rank !== undefined && OutputFieldLabel("排名", rank)}
      {rankOrder && OutputFieldLabel("排名顺序", rankOrder)}
      {segmentId && OutputFieldLabel("段ID", segmentId)}
      {isFinalResult && OutputFieldLabel("最终结果", isFinalResult.toString())}
    </>
  );
};
